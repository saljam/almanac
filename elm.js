...
Elm.Almanac.make = function (_elm) {
   ...
   a2) {
      return function () {
         var factor = 1000;
         var diff = $Basics.floor(a1 * factor) - $Basics.floor(a2 * factor);
         var pi$ = $Basics.floor($Basics.pi * factor);
   ...
   var radius = 150;
   var complement = F2(function (x,
   y) {
      return A2(_op["&-"],x,y);
   })(3 / 2 * $Basics.pi);
   var timeAt = function (angle) {
      return function () {
         var h = complement(angle) * 24 / (2 * $Basics.pi);
         var m = (h - $Basics.toFloat($Basics.floor(h))) * 60;
         return A2($Basics._op["++"],
         $Basics.toString($Basics.floor(h)),
         A2($Basics._op["++"],
         ":",
         $Basics.toString($Basics.floor(m))));
      }();
   };
   ...
   text) {
      return function () {
         var l = len + 50;
         var ra = _U.cmp(angle,
         $Basics.pi / 2) > 0 && _U.cmp(angle,
         3 * $Basics.pi / 2) < 0 ? angle + $Basics.pi : angle;
         return $Graphics$Collage.rotate(ra)($Graphics$Collage.move({ctor: "_Tuple2"
                                                                    ,_0: l * $Basics.cos(angle)
                                                                    ,_1: l * $Basics.sin(angle)})($Graphics$Collage.toForm($Text.plainText(text))));
      }();
   });
   var drawNum = function (n) {
      return function () {
         ...
         return $Graphics$Collage.move({ctor: "_Tuple2"
                                       ,_0: r * $Basics.cos(a)
                                       ,_1: r * $Basics.sin(a)})($Graphics$Collage.toForm($Text.plainText($Basics.toString(n))));
      }();
   };
   var slice = F2(function (start,
   end) {
      return function () {
         var factor = 1000;
         var arclength = $Basics.floor(A2(_op["&-"],
         end,
         start) * factor);
         var makePoint = function (t) {
            return $Basics.fromPolar({ctor: "_Tuple2"
                                     ,_0: radius
                                     ,_1: start + $Basics.toFloat(t) / factor});
         };
         var foo = $Debug.watch("start")(start) + $Debug.watch("end")(end);
         return _U.cmp(arclength,
         25) < 0 ? _U.cmp($Basics.abs(start),
         $Basics.pi / 2) < 0 ? $Graphics$Collage.circle(radius) : $Graphics$Collage.circle(0) : $Graphics$Collage.polygon(A2($List._op["::"],
         {ctor: "_Tuple2",_0: 0,_1: 0},
         A2($List.map,
         makePoint,
         _L.range(0,arclength))));
      }();
   });
   ...
   var clockface = F4(function (sunset,
   dusk,
   nauticalDusk,
   civilDusk) {
      return $Graphics$Collage.group(_L.fromArray([$Graphics$Collage.filled(A3($Color.rgb,
                                                  218,
                                                  237,
                                                  245))($Graphics$Collage.circle(radius))
                                                  ...
                                                  18,
                                                  62,
                                                  124))(A2(slice,0 - dusk,dusk))
                                                  ,$Graphics$Collage.outlined($Graphics$Collage.solid($Color.grey))($Graphics$Collage.circle(radius))
                                                  ,A3(marker,
                                                  $Color.orange,
                                                  radius,
                                                  sunset)
   ...
   var Evening = {ctor: "Evening"};
   var Morning = {ctor: "Morning"};
   var obl = function (t) {
      return $Basics.degrees(23.4393 - 1.3e-2 * t);
   };
   var g = function (t) {
      return $Basics.degrees(357.528 + 35999.05 * t);
   };
   var ec = function (t) {
      return 1.915 * $Basics.sin(g(t)) + 2.0e-2 * $Basics.sin(2 * g(t));
   };
   var l = function (t) {
      return 280.46 + 36000.77 * t;
   };
   var lambda = function (t) {
      return $Basics.degrees(l(t) + ec(t));
   };
   var delta = function (t) {
      return $Basics.asin($Basics.sin(obl(t)) * $Basics.sin(lambda(t)));
   };
   var e = function (t) {
      return $Basics.degrees(0 - ec(t) + 2.466 * $Basics.sin(2 * lambda(t)) - 5.3e-2 * $Basics.sin(4 * lambda(t)));
   };
   var gha = F2(function (t,ut) {
      return ut - $Basics.pi + e(t);
   });
   ...
   $long) {
      return function () {
         var lat$ = $Basics.degrees(lat);
         var t = (date + ut / 360) / 36525;
         var cosc = ($Basics.sin(h) - $Basics.sin(lat$) * $Basics.sin(delta(t))) / ($Basics.cos(lat$) * $Basics.cos(delta(t)));
         var correction = _U.cmp(cosc,
         1) > 0 ? 0 : _U.cmp(cosc,
         -1) < 0 ? $Basics.pi : dir * $Basics.acos(cosc);
         var ut$ = $Basics.pi - $Basics.degrees($long) - e(t) - correction;
   ...
   var timeAtSunAngle = F3(function (dir,
   h,
   date) {
      return function () {
         var h$ = $Basics.degrees(h);
         var date$ = $Basics.toFloat(julian(date) - j2000);
         var dir$ = function () {
            switch (dir.ctor)
   ...
   var sunrise = A2(timeAtSunAngle,
   Morning,
   -0.833);
   var astroDawn = A2(timeAtSunAngle,
   Morning,
   -18);
   var nauticalDawn = A2(timeAtSunAngle,
   Morning,
   -12);
   var civilDawn = A2(timeAtSunAngle,
   Morning,
   -6);
   var sunset = A2(timeAtSunAngle,
   Evening,
   -0.833);
   var noon = F3(function (x,y,z) {
      return function () {
         var set = A3(sunset,x,y,z);
         var rise = A3(sunrise,x,y,z);
         var avg = (rise + set) / 2;
         return _U.eq(rise,
         set) && _U.cmp(rise,
         $Basics.pi / 2) < 0 ? A2(_op["&-"],
         avg,
         $Basics.pi) : avg;
      }();
   });
   ...
   tz) {
      return function () {
         ...
         var nauticalDusk$ = get(nauticalDusk);
         var civilDusk$ = get(civilDusk);
         var sunset$ = $Debug.watch("sunset")(get(sunset));
         var sunrise$ = $Debug.watch("sunrise")(get(sunrise));
         var dawn$ = get(astroDawn);
         return A2($Graphics$Collage.collage,
         500,
         500)(A2($Basics._op["++"],
         _L.fromArray([$Graphics$Collage.rotate(noon$)(A4(clockface,
                      A2(_op["&-"],sunset$,noon$),
                      A2(_op["&-"],dusk$,noon$),
                      A2(_op["&-"],
                      nauticalDusk$,
                      noon$),
                      A2(_op["&-"],civilDusk$,noon$)))
                      ,A2(label,
                      radius,
                      sunset$)(A2($Basics._op["++"],
                      "sunset ",
                      timeAt(sunset$)))
                      ,A2(label,
                      radius,
                      sunrise$)(A2($Basics._op["++"],
                      "sunrise ",
                      timeAt(sunrise$)))
                      ,A2(label,
                      radius,
